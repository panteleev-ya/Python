...
14. Введите данные сервера VPN. Введите IP-адрес сервера из /info в поле `Адрес в Интернете`, затем введите в поле `Имя пункта назначения` описание (короткое название) своего соединения VPN. Затем нажмите `Готово`

15. Нажмите на иконку `Сеть` на панели задач - сверху над доступными Wi-Fi сетями появится настроенное нами VPN-соединение. Жмем `Подключиться`, в появившемся диалоговом окне вводим логин и пароль из /info.
"""
...
7. Введите в поле `Сервер` IP-адрес вашего сервера VPN из /info.

8. Обязательно выберите тип VPN: `IKEv2 EAP (Username/Password)`.

9. Введите в поля `Логин` и `Пароль` введите логин и пароль из /info.
...
8. В поле `Сервер` и `Удаленный ID` введите IP-адрес сервера из /info. В поле `Локальный ID` введите цифру 1.

9. Введите имя пользователя и пароль из /info в разделе `Аутентификация`, а затем нажмите `Готово`.
