...
# Заранее известные константы
    # Общие константы
    dielectric_strength = 3 * 10 ** 6  # Электрическая прочность воздушного промежутка (В/м)
...
# Вычисленные, не изменяющиеся данные
    # Все про провода
    wire_R = wire_p * wire_length / wire_S                                                                              # (R = p*l/S) -> сопротивление каждого провода (они оба одинаковые)
    wire_Q_dt = (battery_I ** 2) * wire_R                                                                               # (Q/t = I^2*R) -> выделяемое количество теплоты на каждом проводе в секунду
    wire_U = battery_I * wire_R                                                                                         # (U = I*R) -> напряжение на каждом проводе (В)

    # Все про воздух
    air_p = atmosphere_pressure * air_M / (gas_R * air_temperature)                                                     # (P = (p*M)/(R*T)) -> плотность воздуха (кг/м^3)

    # Все про дугу
    arc_start_U = battery_U - 2 * wire_U                                                                                # (U = Uобщ - Uп1 - Uп2 = Uобщ - 2*Uп) -> начальное напряжение эл. дуги
    arc_start_E = arc_start_U / stick_d                                                                                 # (E = U/d) -> начальная напряженность поля, образовавшегося между проводниками
    arc_h = 0                                                                                                           # (h = at^2/2, a - ускорение дуги, h0 = 0, v0 = 0) -> высота, на которую поднялась дуга
    arc_p = atmosphere_pressure * arc_M / (gas_R * arc_temperature)                                                     # (P = (p*M)/(R*T)) -> плотность дуги (кг/м^3)
    arc_start_position = [695, 613]                                                                                     # стартовая позиция дуги == [середина между проводниками, самая нижняя точка между проводниками]
    arc_a = (air_p / arc_p - 1) * g                                                                                     # (ma = pgV - mg, p - плотность воздуха => a = (p1/p2 - 1)g) -> ускорение дуги (по оси Y)
    arc_max_h = "undefined"  # cos((stick_angle * pi / 180) / 2)                                                        # h_max = cos(a/2) -> максимальная высота подъема дуги
    ion_a = 1                                                                                                           # (0.2 * (E - 24.5)^2) -> Коэффициент ударной ионизации

    # Условие образования дуги
    required_stick_U = dielectric_strength * stick_d                                                                    # (Up = Ep/d) -> мин. напряжение, при котором образуется эл. дуга
# Обновляющиеся данные
    # Все про проводники
    stick_passed_length = arc_h / cos((stick_angle * pi / 180) / 2)                                                     # (length = h/cos(a/2)) -> длина проводника, которое дуга уже преодолела
    stick_passed_d = stick_d + 2 * arc_h * tan((stick_angle * pi / 180) / 2)                                                      # (2*d = h * tg(a/2)) -> расстояние между проводниками на данной высоте
    stick_passed_R = stick_p * stick_passed_length / stick_S                                                            # (R = p*l/S) -> сопротивление участка проводника, который теперь тоже является участком электрической цепи
    stick_passed_U = battery_I * stick_passed_R                                                                         # (U = I*R) -> напряжение на каждом проводнике (В)

    # Все про дугу
    arc_position = [695, 613]                                                                                           # стартовая позиция дуги == [середина между проводниками, самая нижняя точка между проводниками]
    arc_U = battery_U - 2 * (wire_U + stick_passed_U)                                                                   # (U = Uобщ - 2*U_w - 2*U_s) -> напряжение дуги
    ...
    def make_start_calculations(self):
        ...
        self.arc_start_position = [695, 613]                                                                            # стартовая позиция дуги == [середина между проводниками, самая нижняя точка между проводниками]
    ...
    def get_info(self):
        return [
            f"U: {round(self.battery_U, self.output_precision)}",
            f"I: {round(self.battery_I, self.output_precision)}",
            f"Wires -> R: {round(self.wire_R, self.output_precision)}",
            f"Wires -> Q/t: {round(self.wire_Q_dt, self.output_precision)}",
            f"Arc -> U: {round(self.arc_U, self.output_precision)}",
            f"Arc -> length: {round(self.stick_passed_length, self.output_precision)}",
            f"Arc -> E: {round(self.arc_E, self.output_precision)}",
            f"Arc -> H_max: {round(self.arc_max_h, self.output_precision)}"
        ]
